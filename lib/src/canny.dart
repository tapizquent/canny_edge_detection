...
Set<Set<Index2d>> canny(
  ...
  bool Function(int x, int y) isWeak =
      (x, y) => getSafe(x, y, image) >= lowThreshold!;
  bool Function(int x, int y) isStrong =
      (x, y) => getSafe(x, y, image) >= highThreshold!;
...
int? _otsusMethod(Image image) {
  ...
  for (var currentThreshold = 1; currentThreshold < 255; ++currentThreshold) {
    ...
    for (var i = 0; i < currentThreshold; ++i)
      background_mean += i * histogramm[i];
    background_mean /= bakground_sum;

    double foreground_mean = 0;
    for (var i = currentThreshold; i < 256; ++i)
      foreground_mean += i * histogramm[i];
    foreground_mean /= foreground_sum;

    //calculate between class variance
    final double currentBetweenClassVariance = background_weight *
        foreground_weight *
        math.pow(background_mean - foreground_mean, 2);

    if (maxBetweenClassVariance == null ||
        currentBetweenClassVariance > maxBetweenClassVariance) {
...
class Index2d {
  ...
  bool operator ==(dynamic other) =>
      other is Index2d && other.x == x && other.y == y;
  @override
